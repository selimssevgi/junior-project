...
def add_writer(writers, movie)
  ...
  writers_arr.each do |writer|
    ...
    w = Person.find_by(full_name: full_name)
    if w.nil?
      w = Person.create(full_name: full_name)
    end
    # w.writings.create(movie: movie)
    Cast.create(movie: movie, person: w, role_id: 2)
  end
end
...
def add_actor(actors, movie)
  ...
  actors_arr.each do |actor|
    ...
    a = Person.find_by(full_name: actor)
    if a.nil?
      # this actor doesnt exist in my database, add it
      a = Person.create(full_name: actor)
    end
    # a.actings.create(movie: movie) 
    Cast.create(movie: movie, person: a, role_id: 3)
  end
end
...
def add_director(directors, movie)
  ...
  directors_arr.each do |director|
    ...
    d = Person.find_by(full_name: director)
    if d.nil?
      # this director doesnt exist in my database, add it
      d = Person.create(full_name: director)
    end
    # d.directings.create(movie: movie) 
    Cast.create(movie: movie, person: d, role_id: 1)
  end
end
